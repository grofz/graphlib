...
!
      ...
      case(3) ! puting single item to stack and removing it
        ...
        out = aa % peek()
        if (size(out)==1) then
          call t % assert_eq(out(1)==10, .true., ' 3a. peeking stack correct value')
        else
          call t % assert_eq(.false., .true., ' 3a. wrong dimension')
        end if
        deallocate(out)
        ...
        out = aa % pop()
        if (size(out)==1) then
          call t % assert_eq(out(1)==10, .true., ' 3b. poping stack correct value')
        else
          call t % assert_eq(.false., .true., ' 3b. wrong dimension')
        end if
        deallocate(out)
        call t % assert_eq(aa%empty(), .true., ' 3c. stack is empty again')
      ...
      case(4) ! puting many items to stack and removing in correct order
        ...
        call t % assert_eq(isok, .true., ' 4. put/pop many elements ok')
      ...
      case(5) ! putting items / making copy / removing items / copy still ok
        ...
        call t % assert_eq(isok, .true., ' 5. copied stack ok')
        call t % assert_eq(bb%empty(), .true., ' 5b. stack empty')
