...
! - graph_handle_t and pqueue_handle_t instead just handle_t
  ...
  module graph_mod
    ...
    type, public :: graph_t
      ...
      integer, private :: graph_id
        ! graph_id is used as the initial version of all handles created by
        ! this graph instance
    contains
      ...
      procedure, non_overridable :: get_graph_id => graph_get_graph_id
    end type graph_t
...
!
    ...
    pure integer function graph_get_graph_id(this) result (graph_id)
      class(graph_t), intent(in) :: this
      graph_id = this%graph_id
    end function graph_get_graph_id


    subroutine generate_graph_id(this)
      class(graph_t), intent(inout) :: this
!
! Generate an identifier used to distinguish handles belonging to different
! graph instances.
! The identifier is randomly selected; uniqueness is probabilistic.
!
      real(dp) :: x

      call random_number(x)
      this%graph_id = 1 + int(x * real(huge(this%graph_id)/2, dp))
#ifdef DEBUG
      print '("Generated graph_id = ",i0)', this%graph_id
#endif
    end subroutine generate_graph_id
    ...
    subroutine increase_vertices_capacity(this, new_capacity)
      ...
      block ! create fresh handles
        ...
        do i=old_capacity+1, new_capacity0
          ...
          new_handle%version = this%get_graph_id()
